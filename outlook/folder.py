...
class Folder:
    ...
    @staticmethod
    def _search(emails: List[Email], sender_names: List[str]=[], sender_emails: List[str]=[], subjects: List[str]=[], bodies: List[str]=[], attachments: List[str]=[]) -> List[Email]:
    ...
    def search(self, recursive=False, **kwargs) -> List[Email]:
        ...
        emails = self.emails()
        matches = Folder._search(emails, **kwargs)
